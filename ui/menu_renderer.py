...
class MenuRenderer:
    ...
    def draw_model_selection(self):
        ...
        if not self.ui.available_models:
            ...
            # 선공/후공 선택 (모델 목록 바로 다음)
            self.draw_turn_order_selection()
    ...
    def draw_simulation_slider(self):
        ...
        label_rect = label.get_rect(center=(center_x, 620))
        ...
        self.ui.screen.blit(value_text, (slider_x + slider_width + 20, 635))
    
    def draw_turn_order_selection(self):
        """선공/후공 선택 UI"""
        center_x = self.ui.WINDOW_WIDTH // 2
        mouse_pos = pygame.mouse.get_pos()
        
        # 라벨
        label = self.ui.font_small.render("Turn Order:", True, Colors.TEXT_COLOR)
        label_rect = label.get_rect(center=(center_x, 530))
        self.ui.screen.blit(label, label_rect)
        ...
        # You First 버튼
        first_button = pygame.Rect(center_x - button_width - gap // 2, button_y, button_width, button_height)
        if self.ui.player_goes_first:
            color = Colors.PLAYER1_COLOR
        elif first_button.collidepoint(mouse_pos):
            color = Colors.BUTTON_HOVER_COLOR
        else:
            color = Colors.BUTTON_COLOR
        
        pygame.draw.rect(self.ui.screen, color, first_button, border_radius=8)
        pygame.draw.rect(self.ui.screen, Colors.THICK_GRID_COLOR, first_button, 2, border_radius=8)
        
        first_text = self.ui.font_small.render("You First", True, (255, 255, 255))
        first_text_rect = first_text.get_rect(center=first_button.center)
        self.ui.screen.blit(first_text, first_text_rect)
        
        # AI First 버튼
        second_button = pygame.Rect(center_x + gap // 2, button_y, button_width, button_height)
        if not self.ui.player_goes_first:
            color = Colors.PLAYER2_COLOR
        elif second_button.collidepoint(mouse_pos):
            color = Colors.BUTTON_HOVER_COLOR
        else:
            color = Colors.BUTTON_COLOR
        
        pygame.draw.rect(self.ui.screen, color, second_button, border_radius=8)
        pygame.draw.rect(self.ui.screen, Colors.THICK_GRID_COLOR, second_button, 2, border_radius=8)
        
        second_text = self.ui.font_small.render("AI First", True, (255, 255, 255))
        second_text_rect = second_text.get_rect(center=second_button.center)
        self.ui.screen.blit(second_text, second_text_rect)
        
        # 버튼 정보 저장
        self.ui.turn_order_buttons = [
            (first_button, "player_first"),
            (second_button, "ai_first")
        ]
